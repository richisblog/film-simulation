...
export function Preview({ image, settings, lut, leak }: Props) {
  ...
  return (
    <section className={`preview-stage ${compare ? 'comparing' : ''}`} aria-label={copy.photoPreview}>
      <div className="preview-window" ref={windowRef}>
        {compare && (
          <canvas
            ref={originalCanvas}
            role="img"
            aria-label={copy.originalComparison}
        ...
        <canvas
          ...
          ref={canvas}
          role="img"
          aria-label={compare ? copy.effectComparison : copy.effectPreview}
          style={display ? { width: display.w, height: display.h } : undefined}
        />
        <div className="preview-actions">
          <button type="button" className="compare-button" aria-pressed={compare} onClick={() => setCompare((value) => !value)}>
            {compare ? copy.backToEffect : copy.compareOriginal}
          </button>
          <span className="render-mode">{mode === 'webgl2' ? copy.acceleratedPreview : copy.compatibilityMode}</span>
        </div>
